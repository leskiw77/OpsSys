...
int main(int argc, char **argv) {
    ...
    while (1) {
        char mType;
        if (read(sfd, &mType, 1) != 1) throwError ("CLIENT: reading type failed!");
...
void handleReq() {
    ...
    if (read(sfd, &command, 1) != 1) throwError("CLIENT: reading command failed!");
    if (read(sfd, &ctn, sizeof(int)) != sizeof(int)) throwError("CLIENT: reading ctn failed!");
    if (read(sfd, &op1, sizeof(int)) != sizeof(int)) throwError("CLIENT: reading op1 failed!");
    if (read(sfd, &op2, sizeof(int)) != sizeof(int)) throwError("CLIENT: reading op2 failed!");
...
void sendMessage(char type, int ctn, int result) {
    ...
    if (write(sfd, &mess, sizeof(Message)) != sizeof(Message)) throwError ("CLIENT: sending message failed!");
}
...
void registerClient(char *name, int sfd) {
    ...
    if (read(sfd, &mType, 1) != 1) throwError("Receiving login response failed!");
...
int connectLocal(char *path) {
    ...
    if (localSocket == -1) throwError("CLIENT: creating localSocket failed!");

    if (bind(localSocket, &localAddress, sizeof(sa_family_t)) == -1) throwError("local abstract binding failed!");
...
int connectWeb(int address, short port) {
    ...
    if (bind(webSocket, &webAddress, sizeof(webAddress)) == -1) throwError("CLIENT: webSocket binding failed!");
    webAddress.sin_family = AF_INET;
    webAddress.sin_port = htons(port);
    webAddress.sin_addr.s_addr = htonl(address);
    if (connect(webSocket, &webAddress, sizeof(webAddress)) == -1) throwError("CLIENT: webSocket connecting failed!");
...
int validateType(char *type) {
    if (strcmp(type, "loc") == 0) {
        ...
        throwError("Type must be loc or web!");
        return -1;
    }
}
...
int validateAddress(char *ip) {
    int x = inet_addr(ip);
    if (x == -1) throwError("CLIENT: inet_addr failed! Adrees is wrong");
    return x;
}
