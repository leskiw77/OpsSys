...
int main(int argc, char **argv) {
    if (argc != 3) showUsage();
    if (signal(SIGINT, quitter) == SIG_ERR) throwError("Signal failed!");
    if (atexit(cleanup) != 0) throwError("At exit failed!"); // procedura sprzatajaca
    ...
    if (pthread_create(&pinger, NULL, pingerJob, NULL) != 0) throwError("Creating thread failed!");
    if (pthread_create(&commander, NULL, commanderJob, NULL) != 0) throwError("Creating thread failed!");

    struct epoll_event event;
    while (1) {
        if (epoll_wait(epoll, &event, 1, -1) == -1) throwError("Epoll_wait failed!");
...
void handleNewMessage(int fd) {
    ...
    if (mess.type == LOGIN) {
        ...
        pthread_mutex_lock(&clientsLock); // zablokuj, zeby nie scigac sie z pingerem
        for (int i = 0; i < cN; i++) {
            if (strcmp(clients[i].name, mess.name) ==
                0) { // faktycznie istnieje taki klient, ktory chcialby sie wylogowac
...
void tryRegister(int fd, struct sockaddr *addr, socklen_t aSize, Message *mess) {
    ...
    if (cN == 50) {
        mType = FAILSIZE;
        if (sendto(fd, &mType, 1, 0, addr, aSize) != 1) throwError("Failsize message failed!");
        free(addr);
    } else {
        Client *client = findClient(mess->name);
        if (client != NULL) {
            mType = FAILNAME;
            if (sendto(fd, &mType, 1, 0, addr, aSize) != 1) throwError("Failname message failed!");
            free(addr);
        } else {
            ...
            cN++;
            mType = SUCCESS;
            if (sendto(fd, &mType, 1, 0, addr, aSize) != 1) throwError("Success message failed!");
        }
    }
    pthread_mutex_unlock(&clientsLock);
}
    ...
    while (1) {
        ...
        for (int i = 0; i < cN; i++) {
            if (clients[j].sent != clients[j].rec) {
                ...
                removeClient(
                        j); // usuwam klienta i shiftuje tablice, wiec potem chce jeszcze raz ten sam indeks sprawzic
            } else {
                char mType = PING;
...
int prepareMonitor() {
    ...
    if (epoll == -1) throwError("Creating epoll monitor failed!");

    struct epoll_event event;
    event.events = EPOLLIN | EPOLLPRI;
    event.data.fd = webSocket;
    if (epoll_ctl(epoll, EPOLL_CTL_ADD, webSocket, &event) == -1) throwError("ctl for webSocket failed!");

    event.data.fd = localSocket;
    if (epoll_ctl(epoll, EPOLL_CTL_ADD, localSocket, &event) == -1) throwError("ctl for localSocket failed!");
...
int getWebSocket(short portNum) {
    ...
    if (webSocket == -1) throwError("Creating webSocket failed!");

    if (bind(webSocket, &webAddress, sizeof(webAddress)) == -1) throwError("webSocket binding failed!");
...
int getLocalSocket(char *path) {
    ...
    if (localSocket == -1) throwError("Creating localSocket failed!");

    if (bind(localSocket, &localAddress, sizeof(localAddress)) == -1) throwError("localSocket binding failed!");
