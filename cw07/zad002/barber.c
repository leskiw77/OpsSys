...
int main(int argc, char **argv) {
    if (argc != 2) showUsage();
    if (atexit(clearResources) == -1) throwEx("Barber: atexit failed!");
    if (signal(SIGINT, intHandler) == SIG_ERR) throwEx("Barber: signal failed!");
...
void napAndWorkForever() {
    while (1) {
        if (sem_wait(BARBER) == -1) throwEx("Barber: 0 sops failed!"); // czekaj na obudzenie

        if (sem_post(BARBER) == -1) throwEx("Barber: setting himself as awaken failed!");
        if (sem_post(SLOWER) == -1) throwEx("Barber: freeing client failed!");

        pid_t toCut = takeChair();
        cut(toCut);

        while (1) {
            if (sem_wait(FIFO) == -1) throwEx("Barber: 3 sops failed!");
            ...
            if (toCut != -1) { // jesli istnial, to zwolnij kolejke, ostrzyz i kontynuuj
                if (sem_post(FIFO) == -1) throwEx("Barber: 4 sops failed!");
                cut(toCut);
                ...
                if (sem_wait(BARBER) == -1) throwEx("Barber: 5 sops failed!");

                if (sem_post(FIFO) == -1) throwEx("Barber: 6 sops failed!");
                break;
            }
        }
    }
...
pid_t takeChair() {
    if (sem_wait(FIFO) == -1) throwEx("Barber: 1 sem failed!");

    pid_t toCut = fifo->chair;

    if (sem_post(FIFO) == -1) throwEx("Barber: 2 sem failed!");
...
void prepareFifo(int chNum) {
    ...
    if (shmID == -1) throwEx("Barber: creating shared memory failed!");

    if (ftruncate(shmID, sizeof(Fifo)) == -1) throwEx("Barber: truncating shm failed!");
...
void prepareSemafors() {
    ...
    if (BARBER == SEM_FAILED) throwEx("Barber: creating semafors failed!");

    FIFO = sem_open(fifoPath, O_CREAT | O_EXCL | O_RDWR, 0666, 1);
    if (FIFO == SEM_FAILED) throwEx("Barber: creating semafors failed!");

    CHECKER = sem_open(checkerPath, O_CREAT | O_EXCL | O_RDWR, 0666, 1);
    if (CHECKER == SEM_FAILED) throwEx("Barber: creating semafors failed!");

    SLOWER = sem_open(slowerPath, O_CREAT | O_EXCL | O_RDWR, 0666, 0);
    if (SLOWER == SEM_FAILED) throwEx("Barber: creating semafors failed!");
}
