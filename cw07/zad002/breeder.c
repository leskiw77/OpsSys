...
void getCut(int ctsNum){
    while(ctsCounter < ctsNum){
        if(sem_wait(CHECKER) == -1) throwEx("Client: taking checker failed!");

        if(sem_wait(FIFO) == -1) throwEx("Client: taking FIFO failed!");

        int res = takePlace();

        if(sem_post(FIFO) == -1) throwEx("Client: releasing FIFO failed!");

        if(sem_post(CHECKER) == -1) throwEx("Client: releasing checker failed!");
...
int takePlace(){
    int barberStat;
    if(sem_getvalue(BARBER, &barberStat) == -1) throwEx("Client: getting value of BARBER sem failed!");

    pid_t myPID = getpid();

    if(barberStat == 0){
        if(sem_post(BARBER) == -1) throwEx("Client: awakening barber failed!");
        long timeMarker = getMicroTime();
        printf("Time: %ld, Client %d has awakened barber!\n", timeMarker, myPID); fflush(stdout);
        if(sem_wait(SLOWER) == -1) throwEx("Client: waiting for barber failed!"); // waiting for barber to set his value to 1
...
void prepareFifo(){
    ...
    if(shmID == -1) throwEx("Breeder: opening shared memory failed!");

    //if(ftruncate(shmID, sizeof(Fifo)) == -1) throwEx("Breeder: truncating shm failed!");
...
void prepareSemafors(){
    ...
    if(BARBER == SEM_FAILED) throwEx("Breeder: creating semafors failed!");

    FIFO = sem_open(fifoPath, O_RDWR);
    if(FIFO == SEM_FAILED) throwEx("Breeder: creating semafors failed!");

    CHECKER = sem_open(checkerPath, O_RDWR);
    if(CHECKER == SEM_FAILED) throwEx("Breeder: creating semafors failed!");

    SLOWER = sem_open(slowerPath, O_RDWR);
    if(SLOWER == SEM_FAILED) throwEx("Breeder: creating semafors failed!");
}
