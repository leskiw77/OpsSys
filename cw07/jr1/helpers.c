...
char *concat(const char *one, const char *two) {
    char *newc = malloc(sizeof(char) * (strlen(one) + strlen(two) + 1));
    strcpy(newc, one);
    strcat(newc, two);
    return newc;
}
...
char *concat3(const char *s1, const char *s2, const char *s3) {
    char *newc = malloc(sizeof(char) * (strlen(s1) + strlen(s2) + strlen(s3) + 1));
    strcpy(newc, s1);
    strcat(newc, s2);
    strcat(newc, s3);
    return newc;
...
int getQID(char *path, int ID) {
    ...
    if (key == -1) throwEx("Generation of key failed!");

    int QID = msgget(key, 0);
    if (QID == -1) throwEx("Opening queue failed!");
...
long getMicroTime() {
    ...
    if (clock_gettime(CLOCK_MONOTONIC, &marker) == -1) throwEx("Getting time failed!");
